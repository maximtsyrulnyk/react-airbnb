...
export const Additional = ({data}) => {
  return (
    <Box shadow classname="additional">
      <Heading boarder>Додаткові властивості</Heading>
      <ListItem title="Правила дому">{data.house_rules}</ListItem>
      <ListItem title="Політика скасування">
        {data.cancellation_policy}
        </ListItem>
      <ListItem title="Місцевий транспорт">{data.local_transporation}</ListItem>
      ...
      <ListItem title="Інструкції щодо реєстрації">{data.checkin_instructions}</ListItem>
    </Box>
  );
};